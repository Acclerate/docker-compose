...
```shell
mkdir -p /your/path/nginx/html /your/path/nginx/logs /your/path/nginx/conf /your/path/nginx/conf/conf.d
```
...
```shell
docker cp 161d51f2f6c0:/etc/nginx/nginx.conf /your/path/nginx_80/conf
docker cp 161d51f2f6c0:/etc/nginx/conf.d/default.conf /your/path/nginx_80/conf/conf.d
```
...
```shell
docker run -d -p 81:80 --name nginx_80 -v /your/path/nginx_80/html:/usr/share/nginx/html -v /your/path/nginx_80/conf/nginx.conf:/etc/nginx/nginx.conf -v /your/path/nginx_80/conf/conf.d/default.conf:/etc/nginx/conf.d/default.conf -v /your/path/nginx_80/logs:/var/log/nginx nginx
-d: 以后台模式启动容器 -> 后台运行该容器
-p 81:80: 将nginx容器的 80 端口映射到宿主机的 81 端口       注:部分服务器需要手动去放行端口哦
--name nginx_80: 将容器命名为nginx_80
-v /your/path/nginx_80/html:/usr/share/nginx/html: 将宿主机中创建的 html 目录挂载到容器的 /usr/share/nginx/html 目录
```

### 六、测试访问

#### （1）到宿主机 /your/path/nginx/html 目录下创建 index.html 静态资源

```shell
cd /your/path/nginx/html
...
/your/path/nginx_test/conf/conf.d/default.conf

```
server {
    listen 80;
    location / {
        proxy_pass http://www.example.com:81;  # 配置反向代理
    }
}
```

# upstream模块实现负载均衡 -> vim /your/path/nginx_test/conf/nginx.conf
...
http {
    ...
    # include /etc/nginx/conf.d/*.conf;
  	...
  	server {
        listen       80;
        server_name  www.example.com;#域名名称
    	...
    	# start ---------------------------------------------------------------------------------------------
      	...
      	location / {
            ...
            index  index.html index.htm;
       		#proxy_pass http://example.github.io; # 代理的ip地址和端口号
        ...
        location ^~ /api {  # ^~/api/表示匹配前缀为api的请求
            proxy_pass  http://www.example.com:9528/api/;  # 注：proxy_pass的结尾有/， -> 效果：会在请求时将/api/*后面的路径直接拼接到后面
    	...
    	location ^~ /blog/ {
            proxy_pass  http://example.github.io/blog/;  # ^~/blog/表示匹配前缀是blog的请求，proxy_pass的结尾有/， 则会把/blog/*后面的路径直接拼接到后面，即移除blog
